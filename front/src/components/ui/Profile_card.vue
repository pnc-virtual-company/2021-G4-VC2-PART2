...
<section>
    <!-- Start Card -->
    <v-card class="mx-auto mt-8" max-width="344" dense elevation="4" v-show="noPersonalInfo">
        ...
        <v-row justify="center" style="margin-left: 83%; margin-top: -15px">
            <v-dialog v-model="dialog" persistent max-width="600px">
                ...
                <v-card>
                    <v-card-title class="d-flex justify-center" style="background: #2196F3">
                    ...
                    <v-card-text>
                        <v-container>
                            <v-row>
                                <v-col cols="12" sm="6" md="4" style="margin-top: 10px;">
                                    ...
                                    <div>
                                        ...
                                        <v-menu ref="menu" v-model="menu" :close-on-content-click="false" transition="scale-transition" offset-y min-width="auto">
                                            <template v-slot:activator="{ on, attrs }">
                                                <v-text-field v-model="date" label="Birthday date" solo readonly v-bind="attrs" v-on="on"></v-text-field>
                                            </template>
                                ...
                                <v-col cols="12" sm="6" md="4" style="margin-top: 10px">
                                    <v-select :items="gender" label="Gender" solo required v-model="selectedGender"></v-select>
                                </v-col>
                                <v-col cols="12" sm="6" md="4" style="margin-top: 10px">
                                    <v-select :items="status" label="Marital Status" solo required v-model="selectedStatus"></v-select>
                                </v-col>
                                <v-col cols="12" sm="6" style="margin-top: -30px">
                                    <v-select :items="provinces" label="Province" solo required v-model="selectedProvince"></v-select>
                                </v-col>
                                <v-col cols="12" sm="6" style="margin-top: -30px">
                                    <v-select :items="majors" label="Major" solo required v-model="selectedMajor"></v-select>
                                </v-col>
                                <v-col cols="12" style="margin-top: -30px">
                                    <v-textarea label="Skill" required solo v-model="skill"></v-textarea>
                                </v-col>
                                <v-col cols="12" style="margin-top: -30px">
                                    <v-text-field label="Tel: +855" type="phone" solo required v-model="phone"></v-text-field>
                                </v-col>
                            </v-row>
                        </v-container>
                    </v-card-text>
                    <v-card-actions style="margin-top: -40px">
                        ...
                        <v-btn color="blue darken-1" text @click="dialog = false; saveInfo(); addDetailInfo()">
                            Save
                        </v-btn>
                    </v-card-actions>
                </v-card>
    ...
    <v-card class="mx-auto rounded-lg mt-6" max-width="850" dense elevation="5" height="100%" v-show="personalInfo">
