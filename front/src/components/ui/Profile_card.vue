            ...
            <v-dialog v-model="dialog" persistent max-width="600px">
                ...
                <v-card>
                        ...
                        <v-container>
                                ...
                                <v-col cols="12" sm="6" md="4" style="margin-top: 10px;">
                                    ...
                                    <div>
                                        ...
                                        <v-menu ref="menu" v-model="menu" :close-on-content-click="false" transition="scale-transition" offset-y min-width="auto">
                                            <template v-slot:activator="{ on, attrs }">
                                                <v-text-field v-model="date" label="Birthday date" solo readonly v-bind="attrs" v-on="on" :rules="[(v) => !!v || 'Date is required']"></v-text-field>
                                            </template>
                                            <v-date-picker v-model="date" :active-picker.sync="activePicker" :max="(new Date(Date.now() - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10)" min="1950-01-01" @change="save" :rules="[(v) => !!v || 'Date is required']"></v-date-picker>
                                        </v-menu>
                                    </div>
                                    <!-- Date of Birth End  -->
                                </v-col>
                                <v-col cols="12" sm="6" md="4" style="margin-top: 10px">
                                    <v-select :items="gender" label="Gender" solo required v-model="selectedGender" :rules="[(v) => !!v || 'Gender is required']" ></v-select>
                                </v-col>
                                <v-col cols="12" sm="6" md="4" style="margin-top: 10px">
                                    <v-select :items="status" label="Marital Status" solo required v-model="selectedStatus" :rules="[(v) => !!v || 'Status is required']"></v-select>
                                </v-col>
                                <v-col cols="12" sm="6" style="margin-top: -30px">
                                    <v-select :items="provinces" label="Province" solo required v-model="selectedProvince" :rules="[(v) => !!v || 'Province is required']"></v-select>
                                </v-col>
                                <v-col cols="12" sm="6" style="margin-top: -30px">
                                    <v-select :items="majors" label="Major" solo required v-model="selectedMajor" :rules="[(v) => !!v || 'Major is required']"></v-select>
                                </v-col>
                                <v-col cols="12" style="margin-top: -30px">
                                    <v-autocomplete v-model="skills" :items="existSkill" label="Skill" multiple solo :rules="[(v) => !!v || 'Skill is required']"></v-autocomplete>
                                </v-col>
                                <v-col cols="12" style="margin-top: -30px">
                                    <v-text-field label="Tel: 87 XXX XXX" type="phone" solo required v-model="phone" :rules="[(v) => !!v || 'Tel is required']"></v-text-field>
                                </v-col>
                            </v-row>
                            ...
                            <v-row v-else>
                                <v-col cols="12" sm="6" style="margin-top: 10px">
                                    <v-text-field label="Company:" solo required v-model="company" :rules="[(v) => !!v || 'Company is required']"></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="6" style="margin-top: 10px">
                                    <v-text-field label="Company Website:" solo required v-model="website" :rules="[(v) => !!v || 'Website is required']"></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="6" style="margin-top: -30px">
                                    <v-text-field label="Address:" solo required v-model="address" :rules="[(v) => !!v || 'Address is required']"></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="6" style="margin-top: -30px">
                                    <v-text-field label="Position:" solo required v-model="position" :rules="[(v) => !!v || 'Position is required']"></v-text-field>
                                </v-col>
                                <v-col cols="12" style="margin-top: -30px">
                                    <v-text-field label="HR's name:" solo required v-model="HR_name" :rules="[(v) => !!v || 'Name is required']"></v-text-field>
                                </v-col>
                                <v-col cols="12" style="margin-top: -30px">
                                    <v-text-field label="Email: example@gmail.com" type="email" solo required v-model="company_email" :rules="[(v) => !!v || 'Email is required']"></v-text-field>
                                </v-col>
                                <v-col cols="12" style="margin-top: -30px">
                                    <v-text-field label="Tel: 87 XXX XXX" type="Tel" solo required v-model="company_tel" :rules="[(v) => !!v || 'Tel is required']"></v-text-field>
                                </v-col>
                            </v-row>
                    ...
                    <v-card-actions style="margin-top: -40px">
                        ...
                        <v-btn color="blue darken-1" text @click="dialog = false; personalInfo = true; addDetailInfo(); addCompanyDetailInfo()" v-show="!isCompanyDetailHidden">
                            Save
                        </v-btn>
                    </v-card-actions>
                </v-card>
        ...
        <div class="d-flex ma-1 mt-3">
            ...
            <div class="alumni-info-right">
                ...
                <div class="d-flex mt-4">
                    ...
                    <h5 class="ml-4 mt-2">Skills: {{ userSkill }}</h5>
                </div>
...
export default {
    data: () => ({
        ...
        selectedProvince: '',
        selectedStatus: '',
        existSkill: ['HTML', 'CSS', 'C++', 'C#', 'PHP', 'Python', 'JavaScript', 'Java', 'VueJs', 'Vuetify'],
        skills: [],
        phone: '',
        ...
        gender: ['Male', 'Female'],
        status: ['Single', 'Registered partnership', 'Married', 'Widowed', 'Divorced', 'Separated'],
        majors: ['WEP', 'SNA'],
        provinces: ['Phnom Penh', 'Banteay Meanchey', 'Battambang', 'Kampong Cham', 'Kampong Chhnang', 'Kampong Speu', 'Kampong Thom', 'Kampot', 'Kandal', 'Koh Kong', 'Kep', 'Kratie', 'Mondulkiri', 'Oddar Meanchey', 'Pailin', 'Preah Sihanouk', 'Preah Vihear', 'Pursat', 'Prey Veng', 'Ratanakiri', 'Siem Reap', 'Stung Treng', 'Svay Rieng', 'Takeo', 'Tbong Khmum'],
        // _____________ADD DETAIL_____________ //
        detailInfo: [],
        dateofbirth: '',
        sex: '',
        userMajor: '',
        ...
        userPhoneNumber: '',
        userProvince: '',
        // _____________ADD COMPANY_____________ //
    ...
    methods: {
        ...
        addDetailInfo() {
            ...
            axios.post('usersDetail', userInfo)
                .then(res => {
        ...
        addCompanyDetailInfo() {
            ...
            axios.post('companies', companyInfo)
                .then(res => {
                    console.log(res.data);
