<template>
<section>
    <v-card class="mx-auto rounded-lg" max-width="850" dense elevation="5" height="100%" style="margin-top:100px">
        <!-- Start Dialog -->
        <v-row justify="center" style="margin-left: 83%; margin-top: -15px">
            <v-dialog v-model="dialog" persistent max-width="600px">
                <template v-slot:activator="{ on, attrs }">
                    <div class="edit-btn d-flex justify-end ma-2">
                        <v-btn outlined small fab color="indigo" v-bind="attrs" v-on="on">
                ...
                <v-card>
                    ...
                    <v-card-actions style="margin-top: -40px">
                        ...
                        <v-spacer></v-spacer>
                        <v-btn color="blue darken-1" text @click.prevent="
                              dialog = false;
                              personalInfo = true;
                              setDetailInfo();">Save
                        </v-btn>
                        ...
                        <v-row justify="center" style="margin-left: 83%; margin-top: -15px">
                            <v-dialog v-model="companyDialog" persistent max-width="600px">
                              ...
                              "></v-img>
                                    ...
                                    <v-card-actions style="margin-top: -40px">
                                        <v-btn color="red darken-1" text @click="companyDialog = false">
                                            cancel
                                        </v-btn>
                                        <v-spacer></v-spacer>
                                        <v-btn color="blue darken-1" text @click.prevent="
...
export default {
    data: () => ({
        ...
        // Edit company___________________ //
        isAddCompany: false,
        companyInfo: [],
        website: "",
        address: "",
    ...
    methods: {
        // _______________CONVERT SKILL TO STRING  ____________________
        skillConvert(skills) {
        ...
        // _______________EDIT COMPANY_________________ // 
        setCompanyImg(event) {
        ...
        // ___________SAVE DATE_____________ //
        save(date) {
            this.$refs.menu.save(date);
        },
        // ______________Get User ___________________///
        getUser() {
        ...
        // ___________GET USER DETAIL_________ //
        getuserDetailInfo() {
            axios.get("usersDetail/" + this.userId).then((res) => {
                this.detailInfo = res.data[0];
                this.userDetailId = this.detailInfo.id;
                ...
                this.date = this.detailInfo.birthDate;
                this.selectedMajor = this.detailInfo.major;
                this.selectedGender = this.detailInfo.gender;
                this.selectedStatus = this.detailInfo.maritalStatus;
                this.selectedProvince = this.detailInfo.city;
                this.skills = this.detailInfo.skills;
                this.phone = this.detailInfo.phoneNumber;
                this.selectMajor();
            });
        },
        // ___________GET COMPANY_________ //
        ...
        selectMajor() {
            ...
            } else if (this.selectedMajor === 'SNA') {
                this.existSkill = this.SNASkill
            }
        },
        // _______________________UPDATE PERSONAL INFORMATION__________________________ //
        setDetailInfo() {
            let userInfo = {
                birthDate: this.date,
            ...
            axios
                .post("usersDetail/" + this.userDetailId, userInfo)
                .then(() => {
        ...
        // ________________________UPDATAE COMPANY DETAIL INFORMATION______________________________ //
        setCompanyDetailInfo() {
            ...
            axios
                .post("companies_detail/" + this.userDetailId, companyInfo)
                .then(() => {
                    this.getCompanyDetailInfo();
                })
                .catch((error) => {
